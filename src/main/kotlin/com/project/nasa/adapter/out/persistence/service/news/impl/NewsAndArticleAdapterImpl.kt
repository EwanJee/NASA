package com.project.nasa.adapter.out.persistence.service.news.impl

import com.project.nasa.adapter.out.persistence.repository.news.NewsEntityRepository
...
class NewsAndArticleAdapterImpl(
    private val newsEntityRepository: com.project.nasa.adapter.out.persistence.repository.news.NewsEntityRepository,
) : NewsAndArticleAdapter {
    override fun getByQAndDate(
        topic: String,
        date: LocalDate,
        lang: String,
    ): ResponseNews? {
        ...
        val articlesEn: List<ArticleData> =
            newsEntity.articles_en?.map { article ->
                ArticleData(
                    title = article.title,
                    description = article.description,
                    url = article.url,
    ...
    override fun updateNewsEn(
        ...
        return ResponseNews(
            ...
            articles =
                newsEntity.articles.map { article ->
                    ArticleData(
                        title = article.title,
                        description = article.description,
                        url = article.url,
