package com.project.nasa.adapter.out.persistence.service.apod.impl
...
class ApodAdapterImpl(
    ...
    override fun updateTranslation(
        ...
        val stored =
            apodEntityRepository.findById(id)
                .orElseThrow { IllegalArgumentException("해당 정보가 존재하지 않습니다") }
        // IllegalArgumentException Handling 필요
        if (stored.explanation.isBlank()) return stored.translatedExplanation!!
        stored.updateTranslation(translated)
        return stored.translatedExplanation ?: throw IllegalArgumentException("") // 메세지 필요
    }

    override fun sendImageToEmail(
        id: Long,
        email: String,
    ): ResponseEmail {
        val apod: ApodEntity =
